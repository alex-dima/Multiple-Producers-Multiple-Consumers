...
def main():
    ...
    # sometimes there is no new line between consumer outputs
    output_lines = output_lines.split(")")
    output_lines = [
        line.strip() + ")" for line in output_lines if len(line.strip()) > 0]
    ...
    command = subprocess.Popen(
        ["diff", sorted_output_filename, ref_filename], stdout=subprocess.PIPE)
